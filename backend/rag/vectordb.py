...
def add_document_to_db(
    document_id: str, paragraphs: list[str], embeddings: list[list[float]]
):
    try:
        vectors = [
            {
        ...
        raise HTTPException(
            status_code=404, detail=f"Pinecone indexing failed with error: {e}"
        )
...
def fetch_top_paragraphs(document_id: str, embedding: list[float]) -> list[str]:
    try:
        ...
        raise HTTPException(
            status_code=404, detail=f"Pinecone query failed with error: {e}"
        )
...
def document_exists(document_id: str) -> bool:
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error checking document existence: {str(e)}"
        )
